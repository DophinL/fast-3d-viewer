...
function SceneNode({ object, depth, onChange }: { object: Object3D; depth: number; onChange: () => void }) {
  ...
  return (
    <li>
      <div className="scene-node" style={{ '--depth': depth } as CSSProperties}>
        ...
        <button type="button" className="scene-node__visibility" aria-label={object.visible ? `Hide ${object.name || object.type}` : `Show ${object.name || object.type}`} onClick={() => { object.visible = !object.visible; onChange(); }}>
...
export function SceneTree({ root, onChange }: { root: Object3D; onChange: () => void }) {
