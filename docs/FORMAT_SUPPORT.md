...
| VRM Avatar | `.vrm` | Native | No | Yes | Yes | No | VRM 0.x and 1.0 humanoid rigs, MToon materials, expressions, constraints, spring bones, and retained glTF clips. The dedicated route adds expression, eye-contact, blink, idle, framing, and presenter-pose controls. Legacy forward orientation is normalized. Mesh repair is disabled to avoid silently destroying avatar semantics. |
